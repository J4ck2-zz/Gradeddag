...
func NewGRBC(corer *Core, proposer core.NodeID, round int, callBackChannel chan<- *grbcCallBackReq) *GRBC {
	grbc := &GRBC{
		proposer:        proposer,
		round:           round,
		echoMu:          &sync.Mutex{},
...
func (g *GRBC) processPropose(block *Block) {
	...
	g.transmitor.Send(g.nodeID, core.NONE, echo)
...
func (g *GRBC) processEcho(echo *EchoMsg) {
	...
	if g.echoCnt.Add(1) == int32(g.committee.HightThreshold()) && !g.rflag.Load() {
		...
		g.once.Do(func() {
			...
			g.transmitor.Send(g.nodeID, core.NONE, ready)
			...
			g.callBackChannel <- &grbcCallBackReq{
				...
				grade:     GradeOne,
				tag:       NotifyOutPut,
				reference: g.reference.Load().(map[crypto.Digest]core.NodeID),
			}
		})
	}
